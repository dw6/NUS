...
public class BinaryPrimitiveApplication implements Expression
{
	...
	{
		return (op.equals("+")) ? new IntValue(((IntValue) x1).value + ((IntValue) x2).value)
				: (op.equals("-")) ? new IntValue(((IntValue) x1).value - ((IntValue) x2).value)
						: (op.equals("*")) ? new IntValue(((IntValue) x1).value
								* ((IntValue) x2).value)
								: (op.equals("/")) ?

								new IntValue(((IntValue) x1).value / ((IntValue) x2).value)
										: (op.equals("<")) ? ((((IntValue) x1).value < ((IntValue) x2).value) ? new BoolValue(
												true)
												: new BoolValue(false))
												: (op.equals(">")) ? ((((IntValue) x1).value > ((IntValue) x2).value) ? new BoolValue(
														true)
														...
														: (op.equals("=")) ? ((((IntValue) x1).value == ((IntValue) x2).value) ? new BoolValue(
																true)
																: new BoolValue(false))
																: (op.equals("|")) ? new BoolValue(
																		((BoolValue) x1).value
																				|| ((BoolValue) x2).value)
																		: (op.equals("&")) ? (Value) new BoolValue(
																				((BoolValue) x1).value
																						&& ((BoolValue) x2).value)
																				: (op.equals(".")) ? Store.theStore
																						.get(((RecordValue) x1)
																								.get(((PropertyValue) x2).value))
																						: // op.equals("hasproperty"))
																						new BoolValue(
																								((RecordValue) x1)
																										.containsKey(((PropertyValue) x2).value));
	}
