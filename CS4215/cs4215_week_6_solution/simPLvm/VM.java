...
public class VM extends FixedSizeVM {
	...
	private static int newEnvironment(int size) {
		int node_address = New(TAGS.ENVIRONMENT,
				4+size);
		HEAP[node_address+FIRST_CHILD_SLOT] = 4;
	...
	private static int extend(int env,int byHowMany) {
		...
		if (already_copied(env)) env = HEAP[env+FORWARDINGADDRESS];

		for (int i=HEAP[env+FIRST_CHILD_SLOT]; 
		i <= HEAP[env+LAST_CHILD_SLOT]; i++)
	...
	private static void flip() {
		...
		if (temp_root >= 0) temp_root = copy(temp_root); 
		...
		while (scan < Free) {
			for (int c = HEAP[scan+FIRST_CHILD_SLOT]; 
			c <= HEAP[scan+LAST_CHILD_SLOT]; 
			c++) {
			...
			while (true) {
				...
				case OPCODES.DIV: {
					...
					if (divisor==0) {
						...
						pushOnOperandStack(newInt(dividend
								/
								divisor));
						pc++;
						break;
				...
				case OPCODES.OR: {
					...
					pushOnOperandStack(newBool((i1==TRUE) 
							? TRUE : i2));
					pc++;
					break;
				}
				...
				case OPCODES.AND: {
					int i1 = HEAP[popFromOperandStack() + BOOL_VALUE_SLOT];
					int i2 = HEAP[popFromOperandStack() + BOOL_VALUE_SLOT];
					pushOnOperandStack(newBool((i1==FALSE) 
							? FALSE : i2));
					pc++;
					break;
				}
				...
				case OPCODES.JOF: {
					pc = ( HEAP[popFromOperandStack()+BOOL_VALUE_SLOT] == TRUE
							? pc+1
									: ((JOF)i).ADDRESS );
					break;
				}
				...
				case OPCODES.CALL: {
					...
					int s = HEAP[temp_root+SIZE_SLOT] - HEAP[temp_root+FIRST_CHILD_SLOT];
					for (int j = s - 1; j >= s-n; --j)
						HEAP[temp_root+HEAP[temp_root+FIRST_CHILD_SLOT]+j] 
						     = popFromOperandStack();
					popFromOperandStack();
