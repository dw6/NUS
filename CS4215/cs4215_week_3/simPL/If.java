...
public class If implements Expression
{
	...
	{
		return condition.freeVars().union(thenPart.freeVars()).union(elsePart.freeVars());
	}
	...
	public Expression substitute(String var, Expression replacement)
	{
		return new If(condition.substitute(var, replacement),
				thenPart.substitute(var, replacement), elsePart.substitute(var, replacement));
	}
	...
	public Expression oneStep()
	{
		...
		else if (condition instanceof BoolConstant)
