...
//	private static final int FORWARD_ADDRESS_SLOT;
	...
	private static int extend(int env, int byHowMany)
	{
		...
		for (int i = HEAP[env + FIRST_CHILD_SLOT]; i <= HEAP[env + LAST_CHILD_SLOT]; i++)
	...
	public static String run(INSTRUCTION[] INSTRUCTIONARRAY)
	{
		...
		loop: while (true)
		{
			...
			case OPCODES.PLUS:
			{
				pushOnOperandStack(newInt(HEAP[popFromOperandStack() + INT_VALUE_SLOT]
			...
			{
				pushOnOperandStack(newBool((HEAP[popFromOperandStack() + INT_VALUE_SLOT] < HEAP[popFromOperandStack()
			...
			case OPCODES.LDF:
			{
				pushOnOperandStack(newClosure(((LDF) i).ADDRESS, ((LDF) i).MAXSTACKSIZE));
				pc++;
				break;
			}
			...
			case OPCODES.LDRF:
			{
				int new_closure = newClosure(((LDRF) i).ADDRESS, ((LDRF) i).MAXSTACKSIZE);
			...
			case OPCODES.LD:
			{
				pushOnOperandStack(HEAP[e + HEAP[e + FIRST_CHILD_SLOT] + (((LD) i).INDEX)]);
				pc++;
				break;
			}
