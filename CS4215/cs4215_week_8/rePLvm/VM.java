...
public class VM
{
	...
	// p is the table that the compiler constructs for accessing arrays.
			...
			int divisionByZeroAddress, int invalidRecordAccessAddress)
	{
		...
		loop: while (true)
		{
			...
			case OPCODES.RCD:
			{
				...
				RecordValue recValue = new RecordValue(m, n, idp);
				recValue.i_to_idp = new int[n];
				...
				{
					v_n = os.pop();
					i_n = ((IntValue) os.pop()).value;
					
					recValue.i_to_idp[k] = p[m][i_n] ;
					recValue.entries[p[m][i_n]] = v_n;					
				}
			...
			case OPCODES.DOT:
			{
				int ii = ((IntValue) os.pop()).value;
				RecordValue recValue = ((RecordValue) os.pop());
				int m = recValue.idr;
				int j = p[m][ii];
								
				if (j >= 0)
				{
					os.push(recValue.entries[j]);
				}
				else
				{
					pc = invalidRecordAccessAddress;
					System.err.println("InvalidRecordAccess");
				}
			...
			// THROW to be inserted: NOT IMPLEMENTED ... Yet
			case OPCODES.THROW:
			{
				pc++;
				break;
			...
			// TRY to be inserted: NOT IMPLEMENTED ... Yet
			case OPCODES.TRY:
			{
				pc++;
				break;
			...
			// ENDTRY to be inserted: NOT IMPLEMENTED ... Yet
			case OPCODES.ENDTRY:
			{
				pc++;
				break;
