			...
			if (sizeOfOperands == sizeOfFormals)
			{
				...
				while (operandList.hasMoreElements())
				{
					...
					if (!EqualType.equalType(operandType, formalType))
						throw new TypeError("argument " + position + " has type " + operandType
								+ " in " + this + " but function " + "expects arguments of type "
								+ formalType);
				}
				return operatorFunType.returnType;
			}
			else
				throw new TypeError("operator function in application " + this + " expects "
						+ sizeOfFormals + " argument" + ((sizeOfFormals == 1) ? "" : "s")
						+ ", but gets applied to " + sizeOfOperands + " argument"
						+ ((sizeOfOperands == 1) ? "" : "s"));
		}
		else
			throw new TypeError("operator in application " + this
					+ " does not have function type but " + operatorType);
	}
