...
public class Compiler
{
	...
	public static CompilerResult compile(Expression exp, boolean opt, boolean verb)
	{

		instructions = new Vector<INSTRUCTION>();
		toCompileStack = new Stack<ToCompile>();
		toCompileStack.push(new ToCompile(new TOPLEVEL(), new IndexTable(), exp, "", new DONE()));
		...
		try
		{
			...
			INSTRUCTION[] ia = (INSTRUCTION[]) instructions.toArray(new INSTRUCTION[1]);
	...
	public static void compileAll()
	{
		while (!toCompileStack.empty())
		{
			...
			ADDRESS_INSTRUCTION addressInstruction = toCompile.addressInstruction;
			addressInstruction.ADDRESS = instructions.size();
			indexTable = toCompile.indexTable;
			comp(toCompile.body, toCompile.toInsert, toCompile.funVar);
		}
	}
	...
	protected static void comp(Expression exp, INSTRUCTION toInsert, String funVar)
	{
		...
		if (exp instanceof Let)
		{
			...
			comp(new Application(new Fun(formals, ((Let) exp).body), arguments), toInsert, funVar);
		}
		...
		else
		{
			...
			else if (exp instanceof LookupApplication)
			{
				...
				if (optimized)
				{
					...
					instructions.add(new CALL(((LookupApplication) exp).operands.size()));
			}
			...
			else if (exp instanceof Fun)
			{
				...
				while (e.hasMoreElements())
					newIndexTable.extend((String) e.nextElement());
				toCompileStack
						.push(new ToCompile(i, newIndexTable, ((Fun) exp).body, "", new RTN()));
			}
			...
			{
				instructions.add(new LDCP(((PropertyConstant) exp).value));
			}
			else if (exp instanceof Record)
			{
				compAllAssoc(((Record) exp).associations);
				instructions.add(new RCD((((Record) exp).associations).size()));
			}
			...
			{
				instructions.add(new LDCI(indexTable.access(((Assignment) exp).leftHandSide)));
			...
			else if (exp instanceof ThreadExpression)
			{
				...
				instructions.add(i);
				toCompileStack.push(new ToCompile(i, indexTable,
						((ThreadExpression) exp).expression, "", new ENDTHREAD()));
			}
			else if (exp instanceof Wait)
			{
				instructions.add(new WAIT(indexTable.access(((Wait) exp).varname)));
			}
			else if (exp instanceof Signal)
			{
				instructions.add(new SIGNAL(indexTable.access(((Signal) exp).varname)));
			}
