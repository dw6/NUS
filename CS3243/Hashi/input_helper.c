 ...
 *
 * Sample C code framework for an input helper for Hashiwokakero
 *
...
void writeQueryFile(char* queryfile) {
  ...
  /* Input for the problem, in the form of a list 'shima' (Japanese
     for 'islands' */
  fprintf(fp, "shima([");
  first = TRUE;
  for (r = 0; r < g_numrows; r++) {
    for (c = 0; c < g_numcols; c++) {
      if (g_grid[r][c] >= '0' && g_grid[r][c] <= '8') {
	count = g_grid[r][c] - '0';
	if (first)
	  first = FALSE;
	else
	  fprintf(fp, ", ");
	  fprintf(fp, "[[%d, %d], %d]", c, r, count);
      }
    }
  }
  fprintf(fp, "]).\n");
  fprintf(fp, "gridSize(%d, %d).\n", g_numrows, g_numcols);
  /* Close file. */
  fclose(fp);
}
