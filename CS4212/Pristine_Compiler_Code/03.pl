...
/*
 * Derived from 02.pl
 * High-level language: basic programming constructs: assignment, if, while.
 *                      expressions as inherited from 02.pl
 * Low-level language: three adress code with jumps as in 02.pl (no change here)
 *
 * As before we have:
