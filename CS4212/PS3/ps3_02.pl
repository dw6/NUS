/*
  ...
  My additions are located at lines 399-420 
*/
...
/**
  Modification of the compilation of while. In GCC,
  when 'while' is encountered:- 

  1. A jump is made to the label containing the condition 
  2. The condition is executed. If it succeeds, it jumps
     to the body (located above the condition code).
     If it fails, it exits the 'while' loop.
**/


compile((while B do { S }),Code,R) :-  !,
        newlabel(Lcond), newlabel(Lbody), newlabel(Lout),
        compile(B,CodeB,RB),
        compile(S,CodeS,R),
        C0 = (goto Lcond),
        C1 = (if RB \= 0 goto Lbody),
        C2 = (goto Lout),
        Code = (C0;Lbody::;CodeS;Lcond::;CodeB;C1;C2;Lout::).
