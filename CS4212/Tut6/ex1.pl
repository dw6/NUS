...
% compileHL, compileStmt augmented to count number of return statements
% compileProcedure checks for correct number of returns, depending on return type
...
compileStmt( (while B do { S }), Code,Procs,Globs,TG,Globs,TG,
	...
	compileStmt({S},CS,Procs,Globs,TG,_,_,Locs,TL,_,_,
		    MaxTopIn,MaxTopOut,1,RetLbl,RetIn,RetOut),
	C2 = ( goto LblTop ),
	Code = ( LblTop:: ; CB ; C1 ; CS ; C2 ; LblOut:: ).


compileStmt( {S}, Code,Procs,Globs,TG,Globs,TG,Locs,TL,Locs,
	     TL,MaxTopIn,MaxTopOut,_Lev,RetLbl,RetIn,RetOut) :- !, %%%% <<<<
...
compileProcedure(Pdef,Code,Procs,Glob) :-
	...
	compileStmt(Pbody,CodeBody,Procs,Glob,0,_GlobOut,_TGOut,
		    Locs,0,_LocsOut,_TLOut,0,MaxStack,1,RetLbl,0,RetOut), 
    % Check the number of returns, depending on return type
   	(
   		((Type = void, RetOut = 0));(Type \= void, RetOut > 0) 
   		-> true; writeln('too many/few returns'), abort
   	),
	(   MaxStack == 0
	->  StackAllocation = nop
	...
	writeln('====================================='),
	write('	Number of Returns: '), writeln(RetOut),
	writeln('=====================================').
...
:- Program = (
	  int fact#[x]::{
            if ( x == 0 ) then { int i ; j=10 ; i = 3 ; return 1 ; }
...
% No Return in a int function
% :- Program = (
% 	  int fact#[x]::{
%             if ( x == 0 ) then { int i ; i = 3 ; }
% 	    else { int i ; } ;
% 	  } ;
% 	  int i ;
% 	  i = fact#[1] ;
